...
class FaceRecognition:
    ...
    def get_embedding(self, img, batched=False):
        ...
        if  batched == False:
            ...
            _images = []
            for _img in img:
                assert _img.shape[2] == 3 and _img.shape[0:2] == self.image_size
                _data = cv2.cvtColor(_img, cv2.COLOR_BGR2RGB)
                _data = np.transpose(_data, (2, 0, 1))
                _images.append(_data)
            data = np.stack([e for e in _images])
